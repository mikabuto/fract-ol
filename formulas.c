...
/*   Updated: 2022/07/08 18:33:35 by mikabuto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static int	mandelbrot(t_fractol *f, int x, int y)
{
	int			i;
	t_complex	z;
	...
	while (z.re * z.re + z.im * z.im <= 4 && i < ITER)
	{
...
static int	julia(t_fractol *f, int x, int y)
{
	int			i;
	t_complex	z;
	...
	while (z.re * z.re + z.im * z.im <= 4 && i < ITER)
	{
		z = init_complex(z.re * z.re - z.im * z.im + f->julia_c.re, \
...
static int	celtic(t_fractol *f, int x, int y)
{
	int			i;
	t_complex	z;
	...
	while (z.re * z.re + z.im * z.im <= 4 && i < ITER)
	{
		z = init_complex(fabs(z.re * z.re - z.im * z.im) + f->c.re, \
				2 * z.re * z.im + f->c.im);
		i++;
	}
	return (i);
...
static int	ship(t_fractol *f, int x, int y)
{
	int			i;
	t_complex	z;
